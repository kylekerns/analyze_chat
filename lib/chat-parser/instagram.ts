...
export async function parseChatData(data: InstagramChatData): Promise<ChatStats> {
  ...
  analyzePhrases(data.messages, Object.keys(stats.messagesByUser), stats);
 ...
 * Analyze phrases for common and overused expressions
 */
function analyzePhrases(messages: InstagramMessage[], users: string[], stats: ChatStats): void {
  ...
  messages.forEach(message => {
    if (!message || !message.sender_name || !message.content) return;
    
    // Decode the sender name to match how we've stored users elsewhere
    const user = decodeInstagramEmojis(message.sender_name);
    if (!userPhraseCounts[user]) return; // Skip if user not in our tracking list
    
    // Decode message content
    const text = decodeInstagramEmojis(message.content);
    if (!text || text.trim().length === 0) return;
    
    // Skip system or special messages like "You shared a story."
    if (text.includes("shared a story") || 
        text.includes("sent an attachment") ||
        text.includes("You shared")) {
      return;
    }
    
    // Skip Instagram reaction messages (like "आपके मैसेज पर ❤️ रिएक्शन")
    if (text.includes("रिएक्शन") || 
        text.includes("reaction to") || 
        text.includes("reacted to") ||
        text.includes("मैसेज पर") || 
        text.includes("आपके मैसेज")) {
      return;
    }
